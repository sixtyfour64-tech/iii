...
)
{
	...
	{
		INT nFreq = (INT)(FastSine((FLOAT)t / 200.f) * 1000.f + 100.f);
		FLOAT fSine = FastSine((FLOAT)t / 100.f) * (FLOAT)nSamplesPerSec;
...
VOID
WINAPI
AudioSequence2( // By UltraDasher965
	_In_ INT nSamplesPerSec,
	_In_ INT nSampleCount,
	_Inout_ PSHORT psSamples
)
{
	...
	{
		BYTE bFreq = (BYTE)(t * ((7 & t >> 5) + (15 & t / 62) & (31 & t / 64)));
	...
	for (INT z = 0; z < nCubeRoot; z++)
	{
		for (INT y = 0; y < nCubeRoot; y++)
		{
			for (INT x = 0; x < nCubeRoot; x++)
			{
				...
				psSamples[nIndex] =
					(SHORT)(SquareWave(y + z * x, (nFreq % 50) + 10, nSamplesPerSec) * (FLOAT)SHRT_MAX * .3f) +
					(SHORT)(SawtoothWave(x | z, (150 - (nFreq % 200) / 100) + 800, nSamplesPerSec) * (FLOAT)SHRT_MAX * .3f) +
					(SHORT)(TriangleWave((FLOAT)(x & y & z) + (SquareWave(x + y, nFreq % 50, nSamplesPerSec) * nSamplesPerSec),
						(nFreq % 500) / 1000 + 500, nSamplesPerSec) * (FLOAT)SHRT_MAX * .3f);
			}
		}
	}
}
...
)
{
	...
	for (INT t = 0; t < nSampleCount; t++)
	{
		INT nFreq = (INT)(FastSine((FLOAT)t / (200.f - t / (nSampleCount / 1000))) * 100.f + 100.f);
...
VOID
WINAPI
AudioSequence5( // By UltraDasher965
	_In_ INT nSamplesPerSec,
	_In_ INT nSampleCount,
	_Inout_ PSHORT psSamples
)
{
	...
	for (INT t = 0; t < nSampleCount; t++)
	{
		SHORT sFreq = (SHORT)((t * ((70 & t) + (30 & t / 62) & (31 & t / 63))) - 128);
		psSamples[t] = sFreq;
	}
}
...
)
{
	...
	for (INT t = 0; t < nSampleCount * 2; t++)
	{
		BYTE bFreq = (BYTE)(t ^ t & t >> 9) + (t >> 9) * (t >> 9) * (t >> 1) ^ (t * 100);
		((BYTE*)psSamples)[t] = bFreq;
	}
}
...
)
{
	...
	for (INT t = 0; t < nSampleCount; t++)
	{
		INT nRand = (nSampleCount - t * 5) / 124;
		if (nRand < 24)
			nRand = 24;
		if (!(Xorshift32() % nRand))
		{
		...
		psSamples[t] = (SHORT)(SawtoothWave(t, sRand, nSampleCount) * (FLOAT)SHRT_MAX * .1f)
			...
			(SHORT)(TriangleWave(t, 30, nSampleCount) * (FLOAT)SHRT_MAX);
	}
}
...
)
{
	...
	for (INT y = 0; y < nSquareRoot; y++)
	{
		for (INT x = 0; x < nSquareRoot; x++)
		{
			...
			psSamples[nIndex] =
				(SHORT)(SquareWave(y & x, (nFreq % 50) + 10, nSamplesPerSec) * (FLOAT)SHRT_MAX * .3f);
		}
	}
}
...
)
{
	...
	for (INT t = 0; t < nSampleCount * 2; t++)
	{
		FLOAT w = tanf(2.f, (FLOAT)(t >> 8 & t >> 13));
...
)
{
	...
	for (INT t = 0; t < nSampleCount * 2; t++)
	{
		BYTE bFreq = (BYTE)((t * ((t >> 8 & t >> 3) > (t >> 16 & t))) + ((t * (t >> 8 & t >> 3)) >> (t >> 15 & t)));
		((BYTE*)psSamples)[t] = bFreq;
	}
}
...
)
{
	...
	for (INT t = 0; t < nSampleCount; t++)
	{
		psSamples[t] = (SHORT)(TriangleWave(__rdtsc() % 80, 500, nSampleCount) * (FLOAT)SHRT_MAX * .3f) |
			(SHORT)(SquareWave(__rdtsc() % 800, 1000, nSampleCount) * (FLOAT)SHRT_MAX * .3f) + (SHORT)~t + ((SHORT)t >> 2);
	}
}
...
)
{
	...
	for (INT t = 0; t < nSampleCount; t++)
	{
		psSamples[t] = (SHORT)(SawtoothWave(__rdtsc() % 500, 500, nSampleCount) * (FLOAT)SHRT_MAX * .3f) ^
			((SHORT)(SawtoothWave(t % 100, t % 100, nSampleCount) * (FLOAT)SHRT_MAX * .1f) >> 2);
	}
}
...
)
{
	...
	INT nSquareRoot = (INT)cbrtf((FLOAT)nSampleCount) + 1;
	for (INT y = 0; y < nSquareRoot; y++)
	{
		for (INT x = 0; x < nSquareRoot; x++)
		{
			...
			INT nFreq = (INT)((FLOAT)(y | x) * FastCosine((FLOAT)(y & x) / 100.f));
...
)
{
	...
	for (INT y = 0; y < nSquareRoot; y++)
	{
		for (INT x = 0; x < nSquareRoot; x++)
		{
			...
			psSamples[nIndex] = (SHORT)(SineWave(y % (x + 1), (nFreq % 10) + 10, nSamplesPerSec) * (FLOAT)SHRT_MAX * .3f);
		}
	}
}
...
)
{
	...
	INT nSquareRoot = (INT)tanf((FLOAT)nSampleCount) + 1;
	for (INT y = 0; y < nSquareRoot; y++)
	{
		for (INT x = 0; x < nSquareRoot; x++)
		{
			...
			INT nFreq = (INT)((FLOAT)(y ^ x) * exp(sinf(atanf((FLOAT)(y | x)) / 100.f)) * 2.f);
			psSamples[nIndex] = (SHORT)(SineWave(y - (x % (y + 1)), (nFreq % 1000) + 500, nSamplesPerSec) * (FLOAT)SHRT_MAX * .3f);
		}
	}
}
...
)
{
	...
	INT nCubeRoot = (INT)sqrtf((FLOAT)nSampleCount) + 1;
	for (INT z = 0; z < nCubeRoot; z++)
	{
		for (INT y = 0; y < nCubeRoot; y++)
		{
			for (INT x = 0; x < nCubeRoot; x++)
			{
				...
				INT nFreq = (INT)((FLOAT)(y & x) * tanf((FLOAT)z / (FLOAT)nCubeRoot + (FLOAT)x + (FLOAT)nCounter * (FLOAT)y) * 2.f);
...
)
{
	...
	FLOAT a = FastSine(div) * 20.f;
	FLOAT b = FastCosine(div) * 20.f;
	RGBQUAD rgbDst;
...
)
{
	...
	for (INT y = 0; y < h; y++)
	{
		FLOAT _y = (FLOAT)y / 200.f;

		for (INT x = 0; x < w; x++)
		{
			FLOAT div = (FLOAT)t / 4.f;
			FLOAT a = FastSine(div + _y) * 100.f;
			FLOAT b = FastCosine(div + (FLOAT)x / 200.f) * 400.f;
			...
			DWORD rgb = prgbSrc[v * w + x].r / ((0x101010 | (t & y) | ((t & x) << 8) | (t << 16)) + 1);
			if (!rgb)
			{
				rgb = 2;
			}
...
)
{
	...
	RGBQUAD rgbDst;
	HSLCOLOR hsl;
	FLOAT _t = (FLOAT)t / 50.f;

	for (INT y = 0; y < h; y++)
	{
		FLOAT _y = (FLOAT)y / 55.f;
...
)
{
	...
	{
		hbrBall = CreateSolidBrush(RGB(255, 165, 0));
...
)
{
	...
	FLOAT a = FastCosine(div) * 10.f * PI;
...
)
{
	...
	for (INT i = 0; i < 5; i++)
	{
		...
		StretchBlt(hdcTemp, x - (nNewBlockSize - nBlockSize) / 2, y - (nNewBlockSize - nBlockSize) / 2,
			nNewBlockSize, nNewBlockSize, hdcTemp, x, y, nBlockSize, nBlockSize, SRCPAINT);
	}
}
...
)
{
	...
	for (INT y = 0; y < h; y++)
	{
		for (INT x = 0; x < w; x++)
		{
			...
			hsl = RGBToHSL(rgbDst);
			hsl.h = (FLOAT)fmod((DOUBLE)hsl.h + (DOUBLE)(x) / 100000.0 + 0.05, 1.0);
			hsl.s = 1.f;
...
)
{
	...
	for (INT y = 0; y < h; y++)
	{
		for (INT x = 0; x < w; x++)
		{
			u = x + (INT)a + (INT)(SawtoothWave(y, 100, h) * 16.f);
			v = y + (INT)b;
...
)
{
	...
	for (INT y = 0; y < h; y++)
	{
		FLOAT a = SquareWave(t + y, 100, h) * 100.f;
		...
		for (INT x = 0; x < w; x++)
		{
			...
			rgbDst = prgbSrc[v * w + u];
			rgbDst.rgb = ((rgbDst.b - 1) | ((rgbDst.g + 1) << 4) | ((rgbDst.r - 2) << 16));
			prgbDst[y * w + x] = rgbDst;
		}
	}
}
...
)
{
	...
	for (INT y = 0; y < h; y++)
	{
		FLOAT a = TriangleWave(t * 40 + y, 10, h) * 10.f;
...
)
{
	...
	for (INT y = 0; y < h; y++)
	{
		...
		for (INT x = 0; x < w; x++)
		{
			...
			rgbDst = prgbSrc[v * w + u];
			rgbDst.rgb = ((rgbDst.b + (y)) | ((rgbDst.g + (x)) << 8) | ((rgbDst.r + (x)) << 16));
			prgbDst[y * w + x] = rgbDst;
		}
	}
}
...
)
{
	...
	for (INT y = 0; y < h; y++)
	{
		for (INT x = 0; x < w; x++)
		{
			FLOAT a = coshf(atan2f((FLOAT)((y * 32) & t), (FLOAT)((x * 32) & t))) * tanf((FLOAT)(t | 64)) * 32.f;
			FLOAT b = expf((FLOAT)asin((DOUBLE)t / 10.0) + x);
...
)
{
	...
	HSLCOLOR hsl;
	hsl.h = (FLOAT)fmod((DOUBLE)t / 550.0, 1.0);
	hsl.s = 1.f;
	hsl.l = .5f;
...
)
{
	...
	for (INT y = 0; y < h; y++)
	{
		...
		for (INT x = 0; x < w; x++)
		{
			u = (INT)(x * fabs(fmod((DOUBLE)a - (DOUBLE)(INT)(a * a), 5.0))) + x;
			v = y + (INT)(a * a);

			Reflect2D((PINT)&u, (PINT)&v, w, h);

			rgbDst = prgbSrc[v * w + u];
			rgbDst.b = rgbDst.g & 0xAAAAAA | crRainbow;
			prgbDst[y * w + x] = rgbDst;
		}
	}
}
...
)
{
	...
	{
		rgbDst.g = (Xorshift32() % 256) + 0x010101;
